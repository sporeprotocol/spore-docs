...
To create a public cluster that anyone can pay an unlocking fee to tag spores with it, check out: [Create a Public Cluster](/recipes/Create/create-public-cluster).
...
Clusters are immortal by design, it means all clusters, once created, exist permanently on-chain, and not even their owner can melt them from the blockchain.
